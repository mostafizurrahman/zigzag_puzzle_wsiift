...
open class SweetAlert: UIViewController {
    ...
    fileprivate func resizeAndRelayout() {
        ...
        for button in buttons {
            ...
            buttonRect.append(string.boundingRect(with: CGSize(width: width, height:0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes:[NSAttributedString.Key.font:button.titleLabel?.font ?? UIFont(name: "Sanas-serif", size: 16) ], context:nil))
        }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle) -> SweetAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: "  DISMISS  ")
        return self
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.colorFromRGB(0x307BFA))
        userAction = action
        return self
    }
